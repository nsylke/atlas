module Atlas::Commands
    module Ban
        ...
        command(:ban) do |event, *args|
            ...
            if user.nil?
                event.respond('Please specify a user to be banned.')
                return
            end
            ...
            if Atlas::BOT.member(event.server, user.id).roles.first.position >= Atlas::BOT.member(event.server, Atlas::BOT.profile.id).roles.first.position 
                event.respond('Atlas is not allowed to ban that person.')
                return 
            end

            if Atlas::BOT.member(event.server, user.id).roles.first.position >= event.author.roles.first.position
                event.respond('You are not allowed to ban that person.')
                return
            end

            message = event.respond("Are you sure you want to ban #{user.distinct}?")
            ...
            event.user.await(/^(?:y(es)|n(o))$/i) do |answer|
                ...
                if output =~ /^(?:y(es))$/i
                    message.edit("Successfully banned #{user.distinct} from the server.")
