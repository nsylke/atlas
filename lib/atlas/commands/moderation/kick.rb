module Atlas::Commands
    module Kick
        ...
        command(:kick) do |event, *args|
            ...
            return "Atlas is not allowed to kick that person." if Atlas::BOT.member(event.server, user.id).roles.first.position >= Atlas::BOT.member(event.server, Atlas::BOT.profile.id).roles.first.position
            return "You are not allowed to kick that person" if Atlas::BOT.member(event.server, user.id).roles.first.position >= event.author.roles.first.position
